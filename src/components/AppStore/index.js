...
class AppStore extends Component {
  ...
  getFilteredList = searchedApps => {
    ...
    const appsNewList = searchedApps.filter(each => each.category === activeId)
    return appsNewList
  }
  ...
  getSearchResults = () => {
    ...
    const searchResults = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(changeInput.toLowerCase()),
    )
  ...
  render() {
    const {activeId, changeInput} = this.state
    const searchResults = this.getSearchResults()
    const appsNewList = this.getFilteredList(searchResults)
    return (
